...
/// Wait up to `timeout` for a key press and map it to an [`Action`]. Returns
/// [`Action::None`] on timeout or any key we don't bind, so the caller still
/// redraws (keeping dead-reckoned blips gliding).
pub fn next_action(timeout: Duration) -> std::io::Result<Action> {
